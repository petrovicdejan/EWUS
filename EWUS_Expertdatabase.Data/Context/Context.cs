...
namespace EWUS_Expertdatabase.Data
{
    public class EWUSDbContext : DbContext
    {
        ...
        protected override void OnModelCreating(DbModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<ProjectMeasure>().HasRequired(pm => pm.Project).WithMany(t =>  t.ProjectMeasures).HasForeignKey(d => d.ProjectId).WillCascadeOnDelete(false);
            modelBuilder.Entity<ProjectMeasure>().HasRequired(pm => pm.Measure).WithMany(t => t.ProjectMeasures).HasForeignKey(d => d.MeasureId).WillCascadeOnDelete(false);

            modelBuilder.Entity<ProjectMeasure>().Property(x => x.InvestmenCost).IsOptional();
            modelBuilder.Entity<ProjectMeasure>().Property(x => x.ModificationDate).IsOptional();
            modelBuilder.Entity<ProjectMeasure>().Property(x => x.SubmittedOnDate).IsOptional();
